...
namespace Biozin_Royale_Backend.LogicaNegocio.Implementations
{
    ...
    public class EmailService : IEmailService
    {
        ...
        public async Task EnviarCodigoDesactivar2FAAsync(
            string correoDestino,
            string nombre,
            string codigo,
            string correoRemitente)
        {
            ...
            mensaje.From.Add(new MailboxAddress("Biozin Royale", correoRemitente));
            mensaje.To.Add(MailboxAddress.Parse(correoDestino));
            mensaje.Subject = "Código para desactivar la verificación en dos pasos – Biozin Royale";

            var builder = new BodyBuilder();

            string ruta = Path.Combine(Directory.GetCurrentDirectory(), "EmailTemplates", "TwoFactorDisable.html");
            string html = File.ReadAllText(ruta);
